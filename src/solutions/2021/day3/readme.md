...
Binary parsing, how fun! This one was mostly in making sure we understand all the instructions. I kept getting tripped up by what was being asked, in my haste to hit the leaderboard. Boy, did that backfire!

For Part 1, we just build our `gamma` and `epsilon` rates by finding the most and least common bits.

For Part 2, it's really important to thoroughly read all the instructions before beginning. I initially thought we had to find the `oxygen generator rating` and the `CO2 scrubber rating` for each bit. That doesn't even make sense, since we are actually just looking for a single value and considering each bit already for narrowing down which number is the `rating`.
