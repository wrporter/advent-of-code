...
I got really excited, thinking we were going to build a mini virtual machine again this year. Turns out it's not that exciting, but it was still fun. After completing the problem, I generalized the code between the two parts. I then originally was using a callback for the completion of each cycle, but switched to a generator (using `yield`). I've never used generators before and I liked the slight simplification it made!

### Part 1

1. Run a mini VM starting at the first instruction and register `X` at `1`.
2. For each cycle, determine if the operation is ready to be executed. If it is, execute it. Otherwise, decrement its cycle counter by 1.
3. For each "important" cycle (20, 60, 100, etc.), calculate the signal strength.
4. Return the sum of signal strengths.

### Part 2

1. Run a mini VM starting at the first instruction and register `X` at `1`.
2. Start with a clean CRT monitor with all its pixels dark `.`.
3. For each cycle, determine if the operation is ready to be executed. If it is, execute it. Otherwise, decrement its cycle counter by 1.
4. For each cycle, determine if the pixel is within range of the sprite as determined by register `X`. If it is, write a lit pixel `#`, otherwise keep it dark `.`.
5. Return the result of the CRT monitor.
