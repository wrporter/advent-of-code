...
Oh, you silly monkeys. I'm just impressed with the way we realistically calculate where monkeys are going to throw the items 😂.

### Part 1

1. Parse the initial monkey states. This was a nice time to use regexes. I also stored the operation for each monkey for easy use later.
2. For loop over each round. 
   1. For loop over each monkey's turn.
   2. If the monkey has no items, skip their turn.
   3. Otherwise:
      1. Increase the number of items the monkey has inspected.
      2. Increase the worry by the monkey's operation.
      3. Manage worry about the item.
      4. Determine which monkey the item will be thrown to.
3. Sort the monkeys according to how many items they've inspected, in descending order.
4. Return the top two multiplied together.

### Part 2

Oh, the famous Chinese Remainder Theorem (CRT). There is always at least one problem each year that requires the CRT. But without fail, I always have to look it up. After solving the puzzle I went back through and cleaned up my code to pass in a worry managing function for each part.

The solution to this one was the same as Part 1, but with the CRT twist.

1. Before running rounds, calculate the product of all the mutually prime moduli.
2. Use the product as a modulus for managing worry.
