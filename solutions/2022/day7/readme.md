...
I took this one at a leisurely pace with making and eating breakfast in between, so my times were slower. I also accidentally calculated the required space incorrectly so spent half my time debugging why I wasn't getting the right result until it finally dawned on me what I had done 😅.

### Part 1

1. Parse the input into a tree data structure.
2. Populate sizes of directories.
3. Recurse across the tree and sum any directory sizes under 100,000.
4. Return the sum.
...
1. Parse the input into a tree data structure.
2. Populate sizes of directories.
3. Calculate the minimum space required to fit an update.
4. Recurse across all directories in the tree and find the smallest directory to delete that will free up enough space for the update.
