...
Oh boy, this one was a bit of a pain. There are a lot of great utility functions I wrote in Go to process this kind of problem and help debug. I had to rewrite all of that this year since I switched to TypeScript. There were so many places to go wrong. I often wanted to optimize the code, but later settled on it being simplified so I knew exactly what was going on. For instance, I'd rather have a single way to determine the direction a knot should move rather than 8 if-else statements.

### Part 1 & 2

Both parts can be solved with the same strategy. However, my Part 1 differed in that it did not account for multiple knots and only a tail to chase the head around. So the modification came in moving all other knots to follow previous knots along the rope.

1. Loop over each motion (direction and step count).
2. For each step in a given motion, move the rope.
3. Move the head.
4. Move each knot to catch up to its previous knot.
    - If a previous knot is at a diagonal, the next knot needs to jump to a new spot rather than following the same direction of the previous knot. For example, say we have the following:
        ```
        .2345
        1...6
        .H..7
        ...
        ...s.
        ```
      Moving right would cause the 2nd knot to jump directly down to catch up to the 1st knot that travelled diagonally.
        ```
        ..345
        .2..6
        .1H.7
        ...
        ...s.
        ```
5. Keep track of the positions the tail has moved to.
6. Return the number of visited positions by the tail.
