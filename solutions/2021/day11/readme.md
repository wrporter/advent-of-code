...
This one turned out to be a mixture of a bit of complexity in both the solution and the instructions with how the flashing was supposed to work.

Part 1: Before performing any flashing, first increment the entire grid of octopus energy levels. Then for each octopus that has a level above `9`, make it flash and recursively check flashing on its neighbors. This wasn't too bad. I feel like most of my time was spent thoroughly reading the instructions and writing for loops.

Part 2: I switched from keeping track of a number of octopuses that flashed to just using a set of all that did flash. If the size of that set hits `100` on any given step, then I return the step number and we're done!
