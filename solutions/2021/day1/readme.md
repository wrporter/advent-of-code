...
I decided it wasn't worth it for me to stay up until 10:00pm (when AoC opens in my timezone), but I was super excited for Advent of Code this year! Leading up to it, I did quite a few of the puzzles from previous years. It seems like there are some themes that carry across each year, so it can be helpful to get familiar with the different kinds of problems, especially if you want to compete.
...
For Part 2, we now need to keep track of a rolling window of sums. I did this with a separate array and upon reaching the first 3 values started adding to it. Then we follow the same process as Part 1 by checking for an increase and incrementing a counter.
