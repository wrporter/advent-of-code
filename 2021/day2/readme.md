...
So, I happened to be up until 10:00pm. I just couldn't resist staying up a little longer, especially on the early easy days!

This was yet another more-trivial exercise. Just add to several values based on the command to get your final results. I'm used to crafting engineered solutions and not writing fast throw-away code, so I wrote my usual function to parse the input, when I could have just inlined all the logic in that same for loop. Silly me. There's no leaderboard material in that!

Overall, a fast and fun little puzzle.
...
For Part 1, we keep track of the `depth` and `horizontal` position and just increase or decrease based on the command. Then multiply the result of those.

For Part 2, we make a slight modification to how the commands work and introduce a third value: `aim`. Again, when we are done we just multiply to get our answer.
