...
Part 1: I figured there was probably some smart math way to solve the problem, but did not know where to look. So instead, I ran simulations trying a wide range for the initial velocities and the total time. After trying very large values and my solution taking 25 seconds, it worked!

Part 2: I had to tweak my bounds slightly from Part 1 and the same idea worked. I then went through and modified the ranges to be closer to what they actually needed to be based on the input. The `x` velocity should be greater than 0 so the probe can actually make it to the trench (assuming the trench is in a positive `x` plane), then the maximum can't go past the trench, or we will overshoot it after the first step. We can look at the `y` velocity like we are going up, then it's going to eventually drop. Again, we don't want to shoot below the trench, so we start from the bottom of the trench and can shoot up to the positive value of the bottom so by the 0th step, the next step would take the probe just at the bottom edge of our trench.
