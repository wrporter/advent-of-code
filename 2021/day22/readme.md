...
Part 1: I figured Part 2 was going to scale way up, so I first came up with the right approach and the long approach at once. Then submitted my answers, which is why my delta between the 2 parts is so low (9 seconds). For this part, we can basically walk down each point and flip the points on or off, then count how many are on at the end.

Part 2: This one definitely required a lot more than the brute force approach. I created a struct for the cuboids that could calculate an intersection with other cuboids and their own volume. Then I could keep track of each cube that currently exists and then update each one based on all intersections.
