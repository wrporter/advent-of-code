...
Part 1: This one got me tripped up with the border while enhancing. I decided to apply a border while enhancing and indexed into the previous image by -1 for the x and y coordinates when considering what value the next image should have. The other gotcha is that the sample input varies from the given puzzle input with the 0 index of the algorithm turning the light pixels on and off on each consecutive step.

Part 2: I spent an extra minute cleaning up before running this. My algorithm was good enough without having to make modifications. Woot!
